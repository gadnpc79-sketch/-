...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchComplaints = async () => {
      ...
      if (error) {
        ...
        console.log('DEBUG: Raw complaints fetched:', data?.length, data);
        ...
        const filtered = receiverProfile
          ...
          : (data || []);
        console.log('DEBUG: Filtered complaints for admin:', filtered.length, 'Profile:', receiverProfile);
  ...
  }, [isAdmin, receiverProfile]);

  // 3. Sender Notification Subscription (Non-Admin)
  useEffect(() => {
    if (isAdmin) return;

    const channel = supabase
      .channel('public:complaints:sender')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'complaints' }, (payload) => {
        // Check if this complaint belongs to me
        const myIds = JSON.parse(localStorage.getItem('my_complaint_ids') || '[]');
        if (myIds.includes(payload.new.id)) {
          const statusMap = {
            'received': '접수됨',
            'processing': '처리 중',
            'completed': '처리 완료',
          ...
          const statusText = statusMap[payload.new.status] || payload.new.status;

          playNotificationSound();
          alert(`[알림] 민원(ID: ${payload.new.id})의 상태가 '${statusText}'(으)로 변경되었습니다.`);
        }
      })
      .subscribe();
  ...
  const handleSend = async (data) => {
    ...
    if (!supabaseUrl || supabaseUrl.includes('your-project')) {
      alert('CRITICAL ERROR: Supabase connection is NOT configured. Data will NOT be sent to the admin.');
    ...
    try {
      ...
      if (insertedData && insertedData.length > 0) {
        ...
        const myIds = JSON.parse(localStorage.getItem('my_complaint_ids') || '[]');
        myIds.push(newId);
        localStorage.setItem('my_complaint_ids', JSON.stringify(myIds));
      }
